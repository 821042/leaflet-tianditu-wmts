...
L.TileLayer = L.TileLayer.extend({
    options: {
        minZoom: 0,
        maxZoom: 19,  //加载go服务发布的服务这里改为19。（leaflet源码默认18。）其他配置不变
        maxNativeZoom: null,
        minNativeZoom: null,
        subdomains: 'abc',
        errorTileUrl: '',
...
L.TileLayer.WMTS = L.TileLayer.extend({
    ...
    getTileUrl: function (coords) { // (Point, Number) -> String
        ...
        var tilematrix = ident;  //根据请求url规范，不需要前面的tilematrixSet
        var X0 = this.matrixIds[zoom].topLeftCorner.lng;
        var Y0 = this.matrixIds[zoom].topLeftCorner.lat;
        var tilecol = Math.floor((nw.x - X0) / tilewidth);
        var tilerow = -Math.floor((nw.y - Y0) / tilewidth);
        var url = L.Util.template(this._url, {s: this._getSubdomain(coords)});
        url = url + L.Util.getParamString(this.wmtsParams, url) + "&tilematrix=" + tilematrix + "&tilerow=" + tilerow + "&tilecol=" + tilecol
        return url;
        /*
